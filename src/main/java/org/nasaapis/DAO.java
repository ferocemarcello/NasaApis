...
public class DAO {
    ...
    public String getTableInfoAsHtml(String tableName, Pair<String, String> TableColumns) throws SQLException {
        if(!isConnected()) return "No connection to database. Use /connectToDb";
        else {
            List<String> tot_entries = get(tableName,TableColumns.getFirst());
            StringBuilder toReturn = new StringBuilder();
            toReturn.append("The keys of the db in the table ").append(tableName).append(" are:<br><br>");
            tot_entries.forEach(s-> toReturn.append(s).append("<br>"));
            toReturn.append("<br>The number of entries is ").append(tot_entries.size()).append("<br>");
